...
{:else if gameError}
	<div
		class="bg-red-900/30 p-6 rounded-xl border border-red-800 flex items-center justify-center flex-grow text-red-300"
	>
		{gameError}
	</div>
{:else}
	<div
		class="bg-[#13221e] p-4 rounded-xl shadow-lg border border-[#1e3b33] flex-grow flex flex-col overflow-hidden min-h-[250px] lg:flex"
	>
		<h2
			class="text-md font-semibold text-gray-300 mb-3 text-center tracking-widest uppercase flex-shrink-0"
		>
			Game History
		</h2>

		<div
			class="flex-grow overflow-y-auto custom-scrollbar pr-2 space-y-2 text-sm text-gray-300 font-mono"
		>
			...
			<div class="flex flex-col gap-2 text-xs font-mono pb-2">
				{#each historyBlocks as turn}
					<div class="bg-[#1a332a] p-2 rounded border border-[#2a4d40] flex flex-col gap-2">
						<!-- Turn header -->
						<div
							class="flex justify-between items-center text-gray-500 font-bold border-b border-[#2a4d40] border-opacity-50 pb-1 flex-shrink-0"
						>
						...
						<div class="grid grid-cols-2 gap-2">
							...
							<div class="flex flex-col gap-1 pr-2 border-r border-[#2a4d40] border-opacity-50">
								{#each turn.whiteMoves as m}
									<button
										...
										class="flex items-center justify-start gap-2 px-1.5 py-1 rounded transition {currentMoveIndex ===
										m.index
											? 'bg-green-600 text-white shadow'
											: 'bg-[#0f1d18] text-gray-300 hover:bg-[#2d5849]'} w-full text-left"
									>
										<span
											class="text-base font-sans leading-none opacity-90 w-4 text-center text-gray-200"
											>{m.pieceIcon}</span
										>
										<span>{m.text}</span>
									</button>
							...
							<div class="flex flex-col gap-1 pl-1">
								{#each turn.blackMoves as m}
									...
									>
										<span
											class="text-base font-sans leading-none opacity-90 w-4 text-center text-gray-400 drop-shadow-[0_0_1px_rgba(0,0,0,0.8)]"
											>{m.pieceIcon}</span
										>
										<span>{m.text}</span>
									</button>
