...
<div class="fixed bottom-4 right-4 z-[100] flex flex-col gap-2 pointer-events-none w-full max-w-sm px-4 md:px-0">
    {#each toastStore.toasts as toast (toast.id)}
        <div
            ...
            class="pointer-events-auto flex items-center justify-between p-4 rounded-lg shadow-xl border backdrop-blur-md text-white font-medium
                {toast.type === 'success' ? 'bg-green-900/90 border-green-700/50' :
                 toast.type === 'error' ? 'bg-red-900/90 border-red-700/50' :
