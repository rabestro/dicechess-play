...
<div
	class="flex flex-col justify-center gap-3.5 rounded-2xl border border-border bg-surface p-3.5 lg:flex-1"
>
	{#if dice.length > 0}
		<div class="flex items-center justify-center gap-3" aria-label="Dice">
			{#each dice as d, i (i)}
				<div
					class="flex h-14 w-14 items-center justify-center rounded-xl border border-border bg-dice-surface transition-all duration-300 lg:h-16 lg:w-16
				...
				>
					<img
						...
						alt={d.value}
						class="pointer-events-none h-10 w-10 drop-shadow-md select-none lg:h-12 lg:w-12"
					/>
				</div>
			{/each}
		</div>
	{:else}
		<div class="flex items-center justify-center gap-3" aria-label="Dice">
			{#each Array(3) as _, i (i)}
				<div class="h-14 w-14 rounded-xl border border-border opacity-30 lg:h-16 lg:w-16"></div>
			{/each}
		</div>
		{#if emptyText}
