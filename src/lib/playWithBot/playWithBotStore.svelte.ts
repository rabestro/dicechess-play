...
import { getPieceFromFen, PIECE_TO_UNICODE, deriveChessgroundDests, buildDfen } from '../../utils/fenUtils';
import * as DiceChessEngine from '@rabestro/dicechess-engine';
import type { Key } from '@lichess-org/chessground/types';
import { getDieValue } from '../../utils/fenUtils';
import { toastStore } from '../toastStore.svelte';
import { PlayWithBotBot, setBotDiceChessInstance, resetBotDiceChessInstance } from './playWithBotBot';
...
export class PlayWithBotStore {
  ...
  startNewGame(colorPref: 'white' | 'black' | 'random' = 'white', algo: string = 'greedy') {
    ...
    if (this.bet > 0) {
      if (authStore.user && authStore.user.balance < this.bet) {
        toastStore.error("Insufficient balance! Your balance is 🪙 " + authStore.user.balance + ", but this bet requires 🪙 " + this.bet + ".");
  ...
  private getInitialDice(activeColor: 'w' | 'b'): DieState[] {
    const diceChars = this.parsedDfen.dice.toUpperCase().replace(/[^PNBRQK]/g, '').slice(0, 3);
  ...
  canUserRoll = $derived(this.gameStatus === 'rolling' && this.activeColor === this.playerColor && !this.isAnimatingRoll && this.activeDrawOffer === null && this.activeDoubleOffer === null);
  ...
  legalMovesDests = $derived.by<Map<Key, Key[]>>(() => {
    if (this.gameStatus !== 'playing' || this.activeColor !== this.playerColor || this.activeDrawOffer !== null) {
      return new Map();
    }
  ...
  /** Human Move Handler */
  handleBoardMove(orig: string, dest: string, fenAfterMove?: string) {
  ...
  private handleCastlingDieConsumption(orig: string, dest: string, piece: string): void {
    ...
    if (piece.toLowerCase() === 'k' && orig && dest && Math.abs(orig.charCodeAt(0) - dest.charCodeAt(0)) === 2) {
  ...
  private completeMoveLogic(orig: string, dest: string, promotionStr: string | undefined, dieIndex: number) {
   ...
   async botTurn(bypassDoubleCheck = false) {
     ...
     // Bot Doubling Check
     if (!bypassDoubleCheck && this.mode === 'x2' && (this.cubeOwner === null || this.cubeOwner === this.botColor) && this.bet < this.baseBet * 64) {
       ...
       try {
         if (typeof DiceChess?.shouldBotOfferDouble === 'function') {
           botWantsToDouble = DiceChess.shouldBotOfferDouble(currentDfen, this.bet, { algorithm: this.botAlgorithm });
         }
       } catch (e) {
     ...
     if (this.botCanOfferDraw) {
       const currentDfen = buildDfen(this.currentBoardFen, [], this.botColor);
       try {
         const wantsDraw = typeof DiceChess?.shouldBotOfferDraw === 'function'
    ...
    if (!botHasMoves) {
     ...
     const botMovePromise = this.bot.selectBestMove(this.currentBoardFen, availableDice, this.botAlgorithm);
     ...
     for (const move of botMoves) {
       await new Promise((resolve) => setTimeout(resolve, 1000));
       if ((this.gameStatus as GameStatus) === 'defeat' || (this.gameStatus as GameStatus) === 'victory') return;
  ...
  async offerDraw() {
    if (!this.playerCanOfferDraw || this.activeColor !== this.playerColor || this.gameStatus !== 'playing') {
      return;
    }
  ...
  async offerDouble() {
    ...
    if (this.gameStatus !== 'rolling' || this.activeColor !== this.playerColor || this.activeDoubleOffer !== 'player') {
      this.activeDoubleOffer = null;
      this.startTimer();
      return;
    }
    ...
    try {
      if (typeof DiceChess?.shouldBotAcceptDouble === 'function') {
        botAccepts = DiceChess.shouldBotAcceptDouble(currentDfen, proposedBet, { algorithm: this.botAlgorithm });
      }
    } catch (e) {
  ...
  private triggerInsufficientFundsForfeit() {
    ...
    this.gameStatus = 'defeat';
    botStatsStore.recordResult(this.botAlgorithm, 'loss');
    toastStore.error("The bot offered a double, but you have insufficient funds to accept. Automatic forfeit!");
  ...
  private getLeftTimeMap(): { [playerId: string]: number } {
    if (this.timeLimit === null) return {};
    const whiteId = this.playerColor === 'w' ? `user:${authStore.user?.id || '0'}` : `bot:${this.botAlgorithm}`;
    const blackId = this.playerColor === 'b' ? `user:${authStore.user?.id || '0'}` : `bot:${this.botAlgorithm}`;
    return {
  ...
  private async saveGameRecord(result: number) {
    try {
      ...
      if (this.bet > 0) {
        const playerWon = (this.playerColor === 'w' && result === 1) || (this.playerColor === 'b' && result === -1);
        if (playerWon) {
